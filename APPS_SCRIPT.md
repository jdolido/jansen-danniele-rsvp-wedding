...
3) The script receives form-encoded POST parameters: `firstName`, `lastName`, `name`, `attendance`, `guests`, `highChair`, `highChairCount`, `message`, `companionStatus` (JSON array of `{ name, attending }`).
4) The script searches `Sheet2` (by name) for rows with the same last name and a fuzzy-first-name match; when found, it writes `Y` in column D to mark the RSVP, and records companion attendance in column F (`Y` / `N` per companion) based on the `companionStatus` payload. It returns JSON `{ success: true, rowsUpdated, rowsAppended }`.
