...
3) The script receives form-encoded POST parameters: `firstName`, `lastName`, `name`, `attendance`, `guests`, `message`.
4) The script searches `Sheet2` (by name) for rows with the same last name and a fuzzy-first-name match; when found, it writes `Y` in column C for that row and returns JSON { success: true }.
